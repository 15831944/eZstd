...
Public Class APIMessage
    ...
    Public Delegate Function WndProcDelegate(ByVal hWnd As IntPtr, _
    ...
    Public Shared Function CallWindowProc(lpPrevWndFunc As WndProcDelegate, _
    ...
    ''' <remarks></remarks>
    <DllImport("user32.dll")> _
    Public Shared Function DefWindowProc(ByVal hWnd As IntPtr, _
                                         ByVal uMsg As WindowsMessages, ByVal wParam As IntPtr, _
                                         ByVal lParam As IntPtr) As IntPtr
        'CallWindowProc与DefWindowProc关键的区别在于：
        'CallWindowProc 是发送某个消息给某个处理函数(包括自定义消息处理函数),  而DefWindowProc 是发送某个消息给Windows默认的消息处理函数.
        '如果理解了他们这一点的区别, 就能明白,
...
#Region "  ---  发送消息"
    ...
    Public Shared Function PostThreadMessage(ByVal idThread As UInt32, _
...
#Region "  ---  获取消息"
    ...
    Public Shared Function PeekMessage(ByRef message As NativeMessage, ByVal hWnd As IntPtr, _
